...
with col2:
    st.metric("Average Price", f"R{latest_prices['ClosePrice'].mean():,.2f}")

with col3:
    st.metric("Total Volume", f"{latest_prices['Volume'].sum():,.0f}")
...
with tab1:
    ...
    with col2:
        ...
        most_active = latest_prices.sort_values("Volume", ascending=False)
