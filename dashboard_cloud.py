...
with col2:
    st.metric(
        "Average Price",
        f"R{latest_rows['ClosePrice'].mean():,.4f}"
    )
...
with tab1:
    ...
    with col1:
        ...
        display_gainers["DailyReturn"] = display_gainers[
            "DailyReturn"
        ].round(4)

        display_gainers["ClosePrice"] = display_gainers[
            "ClosePrice"
        ].round(4)
    ...
    with col2:
        ...
        display_active["ClosePrice"] = display_active[
            "ClosePrice"
        ].round(4)
...
with tab2:
    ...
    watchlist["ChangePercent"] = watchlist[
        "ChangePercent"
    ].apply(format_change)
...
with tab3:
    ...
    if not selected_df.empty:
        ...
        c1.metric(
            "Close Price",
            f"R{latest['ClosePrice']:,.4f}"
        )

        c2.metric(
            "High Price",
            f"R{latest['HighPrice']:,.4f}"
        )

        c3.metric(
            "Low Price",
            f"R{latest['LowPrice']:,.4f}"
        )
...
with tab6:
    ...
    if not selected_df.empty:
        ...
        display_range = selected_df.copy()
