...
def load_data():
    ...
    indicators = indicators[indicator_cols]
...
st.markdown(
    "Cloud demo using exported CSV data from the SQL Server financial market database."
)
...
with col2:
    st.metric(
        "Average Price",
        f"R{latest_rows['ClosePrice'].mean():,.2f}"
    )
...
with tab1:
    ...
    losers = latest_rows.sort_values(
        "ChangePercent",
        ascending=True
    ).head(5)
...
with tab5:
    ...
    if not selected_df.empty:
        ...
        high_value = selected_df[
            "HighPrice"
        ].max()

        low_value = selected_df[
            "LowPrice"
        ].min()
